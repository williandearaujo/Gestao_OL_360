...
const EmployeeCertificationModal = ({ 
  isOpen, 
  onClose, 
  onSave, 
  employee,
  ...
  const handleSubmit = (e) => {
    ...
    const payload = {
      ...
      learning_item_id: certification.id,
      ...formData,
  ...
  const handleFileUpload = (e) => {
    ...
    if (file) {
      ...
      // Por enquanto, simulamos o caminho do arquivo
      const fileName = `cert_${employee.id}_${certification.id}_${Date.now()}.pdf`;
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`${certification?.nome} - ${employee?.nome}`}
      size="lg"
    >
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-ol-brand-50 p-4 rounded-lg">
          <div className="flex items-center justify-between">
            <div>
              <h4 className="font-medium text-ol-brand-900">{certification?.nome}</h4>
              <p className="text-sm text-ol-brand-700">
                {certification?.codigo} • {certification?.vendor}
              </p>
            </div>
            <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="grid grid-cols-3 gap-3">
              {['DESEJADO', 'OBRIGATORIO', 'OBTIDO'].map(tipo => (
                <label key={tipo} className="flex items-center">
                  ...
                  />
                  <CertificationBadge status={tipo} />
                </label>
              ))}
            </div>
          </div>

          {/* Data Alvo (para DESEJADO e OBRIGATORIO) */}
          ...
          {/* Data de Expiração (para OBTIDO) */}
          {formData.vinculo === 'OBTIDO' && certification?.validade_meses && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Data de Expiração
              </label>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Validade: {certification.validade_meses} meses
              </p>
            </div>
          )}
...
export default EmployeeCertificationModal;
